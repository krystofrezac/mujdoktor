...
export const ProcedureDeleteConfirmModal = ({ id, name, onClose }) => {
  const { mutate: deleteProcedure, isPending: isDeletePending } = useDeleteProcedureMutation()
  ...
  const handleDelete = () => {
    deleteProcedure({ id }, {
      onSuccess: () => {
        enqueueSnackbar({ message: "Procedure deleted successfully", variant: "success" })
      ...
      onError: () => {
        enqueueSnackbar({ message: "Procedure delete failed", variant: "error" })
      }
    })
  }
  ...
  return (
    <Modal show={show} onHide={onClose}>
      ...
      <Modal.Footer>
        <Button variant="danger" disabled={isDeletePending} onClick={handleDelete}>
          {deleteButtonText}
        </Button>
      </Modal.Footer>
    </Modal>
